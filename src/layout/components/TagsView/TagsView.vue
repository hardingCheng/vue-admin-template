<!--
  ...
  功能描述：
  - 显示用户访问过的页面标签，支持快速切换页面
  - 提供标签管理功能：关闭、刷新、关闭其他、关闭所有
  - 支持固定标签（通过路由 meta.affix 配置）
  - 支持右键菜单操作
  
  主要特性：
  1. 自动记录访问历史：用户访问的页面会自动添加到标签栏
  2. 标签持久化：使用 Pinia store 管理标签状态
  3. 页面缓存：配合 keep-alive 实现页面缓存，提高性能
  4. 标签操作：支持中键关闭、右键菜单管理
  5. 页面刷新：通过重定向路由实现页面刷新功能
  
  使用方式：
  - 在布局组件中引入，通常放置在顶部导航栏下方
  - 可通过系统设置控制其显示/隐藏
-->
<template>
  <div class="tags-view">
